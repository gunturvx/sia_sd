          ...
          <div class="row">
            ...
            <div class="card shadow mb-4">
                ...
                <div class="collapse hide" id="absen">
                  <div class="card-body">
                  <form role="form" action="modul/presensi/detailabsensi.php" method="get" name="postform" enctype="multipart/form-data">
                    <!-- <form role="form" action="?page=presensi&act=detailabsensi" method="post" name="postform" enctype="multipart/form-data"> -->
                  <div class="form-group">
                    <label for="kelas">Kelas</label>
                    <select id="kelas" class="form-control" name="kelas">
                       <?php
                        $sql_kelas=mysqli_query($con, "SELECT * FROM tb_mengajar 
                        INNER JOIN tb_mkelas on tb_mengajar.id_mkelas=tb_mkelas.id_mkelas
                        INNER JOIN tb_master_mapel on tb_mengajar.id_mapel=tb_master_mapel.id_mapel
                        WHERE id_guru='$id_login'");
                        while($data=mysqli_fetch_array($sql_kelas)){
                          echo '<option value="'.$data['id_mengajar'].'">'.$data['nama_kelas'].' - '.$data['mapel'].'</option>';
                        }     
