...
<div class="container-fluid">
    ...
    <div class="row">
 	 ...
 	 -->
                ...
                <div class="card-body">
                    <a target="_blank"
                        href="../guru/modul/rekap/rekap_persemester.php?pelajaran=<?=$_GET['pelajaran'] ?>&bulan=<?=$bulan;?>&kelas=<?=$_GET['kelas'] ?>"
			...
			$qry = mysqli_query($con,"SELECT * FROM tb_presensi
				...
				INNER JOIN tb_thajaran ON tb_mengajar.id_thajaran=tb_thajaran.id_thajaran
				INNER JOIN tb_semester ON tb_mengajar.id_semester=tb_semester.id_semester
				WHERE tb_presensi.id_mengajar='$_GET[pelajaran]' and tb_thajaran.status=1 and tb_semester.status=1
			...
			$tglTerakhir = date('t',strtotime($tglBulan));
			 	 ...
			 	 ?>
                    <div class="alert alert-warning alert-dismissible mt-2" role="alert">
                        <b class="text-warning" style="text-transform: uppercase;">BULAN <?=namaBulan($bulan); ?>
                            <?= date('Y') ?></b>
                        <hr>

                        <p>
                            <a target="_blank"
                                href="../guru/modul/rekap/rekap_bulanxl.php?pelajaran=<?=$_GET['pelajaran'] ?>&bulan=<?=$bulan;?>&kelas=<?=$_GET[kelas] ?>"
                            ...
                            <a target="_blank"
                                href="../guru/modul/rekap/rekap_bulan.php?pelajaran=<?=$_GET['pelajaran'] ?>&bulan=<?=$bulan;?>&kelas=<?=$_GET[kelas] ?>"
                                class="btn btn-default">
                                <span class="btn-label">
                                    <i class="fas fa-print"></i>
                                </span>
                        ...
                        <table width="100%" border="1" cellpadding="2" style="border-collapse: collapse;">
                            <tr>
                                ...
                                <td colspan="<?=$tglTerakhir;?>" style="padding: 8px;">PERTEMUAN KE- DAN BULAN : <b
	...
	$qryAbsen = mysqli_query($con,"SELECT * FROM tb_presensi INNER JOIN tb_siswa ON tb_presensi.id_siswa=tb_siswa.id_siswa
		WHERE MONTH(tgl_absen)='$bulan' AND tb_presensi.id_mengajar='$_GET[pelajaran]'
	 GROUP BY tb_presensi.id_siswa  ORDER BY tb_presensi.id_siswa ASC ");
						...
						?>
                                <td align="center" bgcolor="white">
                                    <?php 
									$ket = mysqli_query($con,"SELECT * FROM tb_presensi WHERE DAY(tgl_absen)='$i' AND id_siswa='$d[id_siswa]' AND id_mengajar='$_GET[pelajaran]' AND MONTH(tgl_absen)='$bulan' GROUP BY DAY(tgl_absen) ");
						foreach ($ket as $h)
						...
						?>
                                <td align="center" style="font-weight: bold;">
                                    <?php 
				$sakit = mysqli_fetch_array(mysqli_query($con,"SELECT COUNT(ket) AS sakit FROM tb_presensi WHERE id_siswa='$d[id_siswa]' and ket='S' and MONTH(tgl_absen)='$bulan' and id_mengajar='$_GET[pelajaran]' "));
				echo $sakit['sakit'];
				...
				?>
                                ...
                                <td align="center" style="font-weight: bold;">
                                    <?php 
				$izin = mysqli_fetch_array(mysqli_query($con,"SELECT COUNT(ket) AS izin FROM tb_presensi WHERE id_siswa='$d[id_siswa]' and ket='I' and MONTH(tgl_absen)='$bulan' and id_mengajar='$_GET[pelajaran]' "));
				echo $izin['izin'];
				...
				?>
                                ...
                                <td align="center" style="font-weight: bold;">
                                    <?php 
				$alfa = mysqli_fetch_array(mysqli_query($con,"SELECT COUNT(ket) AS alfa FROM tb_presensi WHERE id_siswa='$d[id_siswa]' and ket='A' and MONTH(tgl_absen)='$bulan' and id_mengajar='$_GET[pelajaran]' "));
				echo $alfa['alfa'];
